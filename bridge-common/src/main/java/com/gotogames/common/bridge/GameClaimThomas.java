...
public class GameClaimThomas {
	...
	//avec Jerome on a changé cette méthode pour qu'elle retourne un booléen
	//true su on peut faire l'update, faux sinon
	public static boolean updateTrump(List<BridgeCard> j1IN,List<BridgeCard> j2IN) throws GameClaimException
	{
		int pos1 = 0;
		int pos2 = 0;
...
//		}
	...
	/**
	 * Cette méthode permet de savoir si on peut prendre tous les atouts adverses restants
	 * @param j1T les atouts du déclarant
	 * @param j2T les atouts du partenaire du déclarant
	 * @param j3T les atouts des adversaires
	 * @return true si on peut les prendre, faux sinon
	 */
	public static boolean allTrumpsTakeable (List<BridgeCard> j1T, List<BridgeCard> j2T, List<BridgeCard> j3T) 
	{		
		Blocage b = getBlocage(j1T, j2T, j3T);
		int tricks = getNbTricksColor(j1T, j2T, j3T, b);
		if(tricks >= j3T.size())
		{
			return true;
		}
	...
	/**
	 * Cette méthode renvoie le nombre de plis revendiquables en faisant d'abord tomber les atouts adverses
	 * @param game la partie concernée
	 * @return le nombre de plis revendiquables
	 * @throws GameClaimException
	 */
	private static int nbTircksAfterTrumps(BridgeGame game) throws GameClaimException 
	{
        /** Les données **/
        ...
        if((joueurCourant == declarer && j1_c0.isEmpty()) || (joueurCourant == declarerPartner && j2_c0.isEmpty()))
        {
            return getNbTricksColor(j1_c0, j2_c0, j3_c0, b_c0);
        }
        ...
        int toursAtout = j3_c0.size();
        j3_c0.clear();
        if(b_c0 == Blocage._Niveau_0 || b_c0 == Blocage._Niveau_1_A || b_c0 == Blocage._Niveau_1_B)
        {
            if(j1_c0.size() < toursAtout)
            {
                joueurCourant = declarerPartner;
                j1_c0.clear();
                for(int i=0; i<toursAtout; i++)
                {
                    j2_c0.remove(0);
                }
            }
            else if(j2_c0.size() < toursAtout)
            {
            ...
            else
            {
                ...
                if (b_c1 == Blocage._Niveau_5_A || b_c2 == Blocage._Niveau_5_A || b_c3 == Blocage._Niveau_5_A)
                {
                    joueurCourant = declarer;
                }
                else if (b_c1 == Blocage._Niveau_5_B || b_c2 == Blocage._Niveau_5_B || b_c3 == Blocage._Niveau_5_B)
                {
                    joueurCourant = declarerPartner;
                }
            }
        }
        else if(b_c0 == Blocage._Niveau_2_A)
        {
            joueurCourant = declarer;
            for(int i=0; i<toursAtout; i++)
            {
                j1_c0.remove(0);
                if(!j2_c0.isEmpty()) j2_c0.remove(0);
            }
        }
        else if(b_c0 == Blocage._Niveau_2_B)
        {
            joueurCourant = declarerPartner;
            for(int i=0; i<toursAtout; i++)
            {
                j2_c0.remove(0);
                if(!j1_c0.isEmpty()) j1_c0.remove(0);
            }
        }
        else if(b_c0 == Blocage._Niveau_3_A || b_c0 == Blocage._Niveau_3_B)
        {
            if(j1_c0.size() < toursAtout)
            {
                joueurCourant = declarerPartner;
        ...
        else if(b_c0 == Blocage._Niveau_4_A)
        {
            joueurCourant = declarer;
            for(int i=0; i<toursAtout; i++)
            {
        ...
        else if(b_c0 == Blocage._Niveau_4_B)
        {
            joueurCourant = declarerPartner;
            for(int i=0; i<toursAtout; i++)
            {
        ...
        else if(b_c0 == Blocage._Niveau_5_A && joueurCourant == declarer)
        {
            for(int i=0; i<toursAtout; i++)
            {
                j1_c0.remove(0);
        ...
        else if(b_c0 == Blocage._Niveau_5_B && joueurCourant == declarerPartner)
        {
            for(int i=0; i<toursAtout; i++)
            {
                j2_c0.remove(0);
        ...
        /**Les plis prennables où l'atout n'intervient pas **/
		...
		/*If there is a level 5 face to a trump's communication --> we can take all the tricks, but we lose a ruff trick*/
                ...
                (	(b_c0 == Blocage._Niveau_0 || b_c0 == Blocage._Niveau_1_B || b_c0 == Blocage._Niveau_2_B || b_c0 == Blocage._Niveau_3_B || b_c0 == Blocage._Niveau_4_B)
                        && (b_c1 == Blocage._Niveau_5_B || b_c2 == Blocage._Niveau_5_B || b_c3 == Blocage._Niveau_5_B))
                        || ((b_c0 == Blocage._Niveau_0 || b_c0 == Blocage._Niveau_1_A || b_c0 == Blocage._Niveau_2_A || b_c0 == Blocage._Niveau_3_A || b_c0 == Blocage._Niveau_4_A)
                        && (b_c1 == Blocage._Niveau_5_A || b_c2 == Blocage._Niveau_5_A || b_c3 ==Blocage._Niveau_5_A)))
        {
            /**MAJ DES ATOUTS : on enleve un atout de chaque main, le plus gros du mort*/
            if(updateTrump(j2_c0, j1_c0)) tr_tr += 1;
            ad_tr = n_c1+n_c2+n_c3+n_u_c1+n_u_c2+n_u_c3;
        }

        else if ((b_c0 == Blocage._Niveau_5_B && (b_c1 == Blocage._Niveau_5_B || b_c2 == Blocage._Niveau_5_B || b_c3 == Blocage._Niveau_5_B))
                || (b_c0 == Blocage._Niveau_5_A && (b_c1 == Blocage._Niveau_5_A || b_c2 == Blocage._Niveau_5_A || b_c3 == Blocage._Niveau_5_A)))
        {
            ad_tr = n_c1+n_c2+n_c3+n_u_c1+n_u_c2+n_u_c3;
        }
	...
	public static int getTotalNbTricks(BridgeGame game) throws GameClaimException
	{
		if(game != null)
		{
			...
			/*Case trump */
				...
				/****Study on trumps****/
				//TODO : Les modifications apportées dans le cas où les adversaires ont des atouts
				if(j3_c0.size() > 0)
				{
					if(joueurCourant == declarer || joueurCourant == declarerPartner)
					{
						if(allTrumpsTakeable(j1_c0, j2_c0, j3_c0))
						{
							return nbTircksAfterTrumps(game);
						} else {
							return getNbTricksColor(j1_c0, j2_c0, j3_c0, b_c0);
						}
					} else {
				...
				/*If there is a level 5 face to a trump's communication --> we can take all the tricks, but we lose a ruff trick*/
				//Modification apportee au niveau du updateTrump : 
				else if ( 
						(	(b_c0 == Blocage._Niveau_0 || b_c0 == Blocage._Niveau_1_B || b_c0 == Blocage._Niveau_2_B || b_c0 == Blocage._Niveau_3_B || b_c0 == Blocage._Niveau_4_B) 
							&& (b_c1 == Blocage._Niveau_5_B || b_c2 == Blocage._Niveau_5_B || b_c3 == Blocage._Niveau_5_B))
						|| ((b_c0 == Blocage._Niveau_0 || b_c0 == Blocage._Niveau_1_A || b_c0 == Blocage._Niveau_2_A || b_c0 == Blocage._Niveau_3_A || b_c0 == Blocage._Niveau_4_A) 
