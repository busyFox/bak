 ...
 * Same as SessionMemoryMgr, but just loginID is not a long but a string ! Use loginIDstr from Session !!
 */
public class SessionMemoryMgr2 extends SessionMgr{
    ...
    private ConcurrentHashMap<String, String> mapLoginID = new ConcurrentHashMap<String, String>();
    private Logger log = LogManager.getLogger(this.getClass());

    public SessionMemoryMgr2(String name) {
        super(name);
    }
    ...
    public boolean deleteSession(String sessionID) {
        ...
        if (session != null) {
            ...
            mapLoginID.remove(session.getLoginIDstr());
            return true;
        }
        return false;
    }
...
//			invalidate(session);
    ...
    public boolean putSession(Session session) {
        if (session != null) {
            ...
            mapLoginID.put(session.getLoginIDstr(), session.getID());
            return true;
        }
        return false;
    }
